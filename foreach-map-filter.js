...
function vowelCount(str){
   ...
   let wordSeperated = [];
   for(let x of str){
        wordSeperated.push(x.toLowerCase());
    }
   wordSeperated.forEach(function(letter) {
...
function valTimesIndex(arr){
    let index = 0;
    let product = arr.map(function(num) {
        let final = num * index;
        index++;
        return final;  
    })
...
function extractKey(arr, key){
    let values = arr.map(function(val) {
        return val[key];
    })
    return values;
}
...
function filterByValue(arr, key) {
    let values = arr.filter(function(val) {
